...
# from models.yolov6s_mobilenetv3 import YoloV6
# from models.yolov6_ghostnet import YoloV6
# from models.yolov6_resnet18spool import YoloV6
# from models.yolov6_resnet18 import YoloV6
from models.yolov6_repvgg import YoloV6
import shutil
...
def train(hyp, opt, device, tb_writer=None, wandb=None):
    ...
    wdir = save_dir / 'weights'  # weights directory
    ...
    if pretrained:
        ...
        model = YoloV6(opt.data or ckpt['model'].yaml, ch=3).to(
            device)  # create
    ...
    for epoch in range(start_epoch, epochs):
        ...
        for i, (imgs, targets, paths, _) in pbar:
            ...
            if rank in [-1, 0]:
                ...
                # Plot
                if ni < 3:
                    f = str(save_dir / ('train_batch%g.jpg' % ni))  # filename
                    result = plot_images(
        ...
        if rank in [-1, 0]:
            ...
            if not opt.notest or final_epoch:  # Calculate mAP
                if final_epoch:  # replot predictions
                    [os.remove(x) for x in glob.glob(
                        str(save_dir / 'test_batch*_pred.jpg')) if os.path.exists(x)]
            ...
            if save:
                ...
                if best_fitness == fi:
                    ...
                    torch.save({'model': ema.ema}, best_f32)
                del ckpt
        # end epoch ----------------------------------------------------------------------------------------------------
    # end training
