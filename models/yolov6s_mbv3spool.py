...
Add ASFF on MobilenetV3 after PANet.

maybe we need also try BiFPN?
...
class YoloV6(nn.Module):
    ...
    def __init__(self, cfg, ch=3):
        ...
        # cd = 1
        cd = 2 # bigger model smaller
        wd = 3

        # self.backbone = mobilenetv3_large(width_mult=0.75, fpn_levels=[5, 9, 14])
        # set focus=True is not work! close it, we that we can have a very small model
        # self.backbone = MobileNetV3_Small(fpn_levels=[2, 7, 10], focus=True)
        self.backbone = MobileNetV3_Small(fpn_levels=[2, 7, 10])

        # base channel
        bc = 24        
        # PANet
        self.conv5 = ConvBase(4*bc, 2*bc)
        self.up1 = nn.Upsample(scale_factor=2)
        self.csp5 = SimBottleneckCSP(4*bc, 2*bc, n=3//wd, shortcut=False)

        self.conv6 = ConvBase(2*bc, bc)
        self.up2 = nn.Upsample(scale_factor=2)
        self.csp6 = SimBottleneckCSP(2*bc, bc//cd, n=3//wd, shortcut=False)

        self.conv7 = ConvBase(bc//cd, bc, 3, 2)
        self.csp7 = SimBottleneckCSP(2*bc, 2*bc//cd, n=3//wd, shortcut=False)

        self.conv8 = ConvBase(2*bc//cd, 2*bc, 3, 2)
        self.csp8 = SimBottleneckCSP(2*bc, 4*bc//cd, n=3//wd, shortcut=False)

        self.detect = Detect(self.nc, self.anchors, [bc//cd, 2*bc//cd, 4*bc//cd])
    ...
    def forward(self, x, augment=False):
        ...
        feas, p3, p4, p5 = self.backbone(x)
        # feas = self.conv_compen(feas)
        # p5 = self.conv_compen(p5)
